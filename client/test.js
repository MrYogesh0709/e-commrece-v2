...
const filterProducts = [...new Set(products.map((product) => product.brand))];
// console.log(filterProducts);
const newCategoryOptions = Array.from(filterProducts, (item, index) => {
  const categoryLabel = filterProducts[index].split("-").join(" ");
  return {
    value: filterProducts[index],
    label: categoryLabel.charAt(0).toUpperCase() + categoryLabel.slice(1),
    checked: false,
  };
});
// console.log(newCategoryOptions);
...
for (const item of newProducts) {
  ...
  data.push({ discountPrice: +discountedPrice, ...item });
}
